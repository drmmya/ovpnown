...
# ---------- OS CHECK ----------
if [[ ! -e /etc/os-release ]]; then echo "Unsupported OS"; exit 1; fi
. /etc/os-release
...
show_connected() {
    ...
    cat /var/log/openvpn-status.log || echo "No status yet."
}
...
wipe_all() {
    ...
    apt-get purge -y openvpn easy-rsa nginx iptables-persistent || true
    rm -rf /etc/openvpn
    rm -f "$WEB_ROOT/openvpn.ovpn"
